...
def get_llm(
    ...
    if source == "OpenAI":
        ...
        # Configure Bedrock with proper credentials
        bedrock_config = {
            "model": model,
            "temperature": temperature,
            "stop_sequences": stop_sequences,
        ...
        # Use access key from environment if available
        aws_access_key = os.getenv("AWS_BEDROCK_ACCESS_KEY")
        if aws_access_key:
            import base64
            # Decode the base64 encoded key
            try:
                decoded_key = base64.b64decode(aws_access_key).decode('utf-8')
                key_parts = decoded_key.split(':')
                if len(key_parts) == 2:
                    bedrock_config["aws_access_key_id"] = key_parts[0]
                    bedrock_config["aws_secret_access_key"] = key_parts[1]
            except Exception:
                # If decoding fails, fall back to environment AWS credentials
                pass
