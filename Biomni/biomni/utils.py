...
def check_and_download_s3_files(
    ...
    def download_with_progress(url: str, file_path: str, desc: str) -> bool:
        ...
        try:
            # Check if this is a private S3 bucket that requires AWS credentials
            aws_access_key = os.getenv("AWS_ACCESS_KEY_ID")
            aws_secret_key = os.getenv("AWS_SECRET_ACCESS_KEY")
            private_bucket_url = os.getenv("MY_S3_BUCKET_URL")
            
            if private_bucket_url and url.startswith(private_bucket_url) and aws_access_key and aws_secret_key:
                try:
                    import boto3
                    ...
                    print(f"✓ Downloaded {desc} from private S3")
                    return True
                    
                except (ImportError, NoCredentialsError) as e:
                    print(f"⚠ S3 authentication failed for {desc}, falling back to HTTP: {e}")
                    # Fall back to HTTP download
                except Exception as e:
                    print(f"⚠ S3 download failed for {desc}, falling back to HTTP: {e}")
                    # Fall back to HTTP download
