...
# Import rich data extraction capabilities
import sys
import os
from pathlib import Path

# Add UI path for rich data extractor  
# Try multiple possible paths to find the UI directory
ui_paths = [
    Path(__file__).parent.parent.parent / "ui",  # When run from package
    Path(__file__).parent.parent / "ui",         # Alternative path
    Path.cwd() / "ui",                           # From current working directory
    Path("/home/jayantlohia16/fresh-start/ui")   # Absolute path fallback
]
...
for ui_path in ui_paths:
    if ui_path.exists() and str(ui_path) not in sys.path:
        ...
        try:
            from rich_data_extractor import RichDataExtractor
            _RICH_DATA_AVAILABLE = True
            print(f"✅ Rich data extractor loaded from: {ui_path}")
            break
        except ImportError:
            continue

if not _RICH_DATA_AVAILABLE:
    print("⚠️ Rich data extractor not found in any UI path - using basic streaming")
    # Remove any added paths that didn't work
    for ui_path in ui_paths:
        if str(ui_path) in sys.path:
            sys.path.remove(str(ui_path))
...
class A1:
    def __init__(
        ...
        Args:
            ...
            checkpointer: Shared MemorySaver for session persistence

        """
        # CRITICAL FIX: Store checkpointer immediately to prevent scoping issues
        ...
        if checkpointer is None:
            checkpointer = None  # Will create new MemorySaver later
        ...
        # Use custom S3 bucket if configured, otherwise use default
        ...
        # Initialize rich data extractor for enhanced streaming
        if _RICH_DATA_AVAILABLE:
            self.rich_data_extractor = RichDataExtractor()
            print("✅ Rich data extraction enabled for enhanced streaming")
        else:
            self.rich_data_extractor = None
            print("⚠️ Rich data extraction not available - using basic text streaming")
...
- Note on using R packages and Bash scripts:
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        # CRITICAL FIX: Use InMemorySaver for development (research finding)
        if self._provided_checkpointer is not None:
            self.checkpointer = self._provided_checkpointer
            print(f"🧠 CONTEXTUAL: Using provided session InMemorySaver")
        else:
            self.checkpointer = InMemorySaver()
            print(f"🧠 CONTEXTUAL: Created new InMemorySaver")

        # Compile the workflow with the correct checkpointer
        self.app = workflow.compile(checkpointer=self.checkpointer)
        print(f"🧠 CONTEXTUAL: Compiled app with checkpointer")
    ...
    def go(self, prompt):
        ...
        for s in self.app.stream(inputs, stream_mode="values", config=config):
            ...
            # 🚀 ENHANCED: Print rich data during regular go() execution too
            if self.rich_data_extractor:
                try:
                    rich_data = self.rich_data_extractor.extract_rich_data(message, s)
                    
                    # Print rich information to console for debugging
                    print(f"\n🔧 RICH DATA STEP {rich_data.step_number}:")
                    print(f"  Message type: {rich_data.message_type}")
                    print(f"  Has code: {rich_data.has_code_execution}")
                    print(f"  Has tools: {rich_data.has_tool_calls}")
                    print(f"  Has files: {rich_data.has_file_operations}")
                    print(f"  Has todos: {rich_data.has_todo_updates}")
                    
                    if rich_data.code_blocks:
                        print(f"  Code blocks: {len(rich_data.code_blocks)}")
                        for i, cb in enumerate(rich_data.code_blocks):
                            print(f"    Block {i+1}: {cb['language']} ({cb['complexity']})")
                    
                    if rich_data.todo_items:
                        print(f"  Todo items: {len(rich_data.todo_items)}")
                        for todo in rich_data.todo_items:
                            status = "✅" if todo['is_completed'] else "⏳"
                            print(f"    {todo['number']}. {status} {todo['description']}")
                    
                    if rich_data.file_operations:
                        print(f"  File operations: {len(rich_data.file_operations)}")
                        for fop in rich_data.file_operations:
                            print(f"    {fop['operation']}: {fop['file_name']}")
                
                except Exception as e:
                    print(f"⚠️ Rich data extraction error: {e}")

        return self.log, message.content

    def set_session_context(self, session_id: str):
        """Set the session context for maintaining conversation history"""
        self.session_id = session_id
        print(f"🧠 CONTEXTUAL: Session context set to {session_id}")
    ...
    def go_stream(self, prompt) -> Generator[dict, None, None]:
        ...
        # CRITICAL FIX: Use session_id as thread_id for persistent context
        thread_id = getattr(self, 'session_id', 'default_session')
        config = {"recursion_limit": 500, "configurable": {"thread_id": thread_id}}
        print(f"🧠 CONTEXTUAL: Using thread_id '{thread_id}' for conversation context")

        # CRITICAL FIX: Let LangGraph handle state loading automatically
        # The checkpointer automatically loads conversation history when using the same thread_id
        inputs = {"messages": [HumanMessage(content=prompt)], "next_step": None}
        print(f"🧠 CONTEXTUAL: LangGraph will auto-load conversation history for thread '{thread_id}'")
        ...
        for s in self.app.stream(inputs, stream_mode="values", config=config):
            ...
            # 🚀 ENHANCED: Extract rich structured data while preserving real-time streaming
            if self.rich_data_extractor:
                print(f"🔧 CALLING rich data extractor for message type: {message.type}")
                
                # Extract comprehensive rich data
                rich_data = self.rich_data_extractor.extract_rich_data(message, s)
                
                print(f"🔧 Rich data extracted - has_code: {rich_data.has_code_execution}")
                print(f"🔧 Parsed content keys: {list(rich_data.parsed_content.keys()) if rich_data.parsed_content else 'None'}")
                
                # 🎯 OPTIMAL: Create lightweight JSON IMMEDIATELY - NO BLOCKING
                lightweight_json = self._create_lightweight_json(rich_data)
                
                yield {
                    "output": lightweight_json,  # Lightweight JSON objects
                    
                    # 🎯 RICH DATA PAYLOAD:
                    "rich_data": rich_data,
                    "message_obj": message,  # Raw LangChain message object
                    "agent_state": s,        # Full LangGraph state
                    
                    # 🧬 STRUCTURED EXTRACTION:
                    "message_type": rich_data.message_type,
                    "step_number": rich_data.step_number,
                    "has_code": rich_data.has_code_execution,
                    "has_tools": rich_data.has_tool_calls,
                    "has_files": rich_data.has_file_operations,
                    "has_data": rich_data.has_data_structures,
                    "has_todos": rich_data.has_todo_updates,
                    
                    # 🔬 SCIENTIFIC CONTENT:
                    "tool_calls": rich_data.tool_calls or [],
                    "tool_results": rich_data.tool_results or [],
                    "code_blocks": rich_data.code_blocks or [],
                    "file_operations": rich_data.file_operations or [],
                    "data_structures": rich_data.data_structures or {},
                    "todo_items": rich_data.todo_items or [],
                    "scientific_data": rich_data.scientific_data or {},
                    
                    # 📊 METADATA:
                    "execution_metadata": rich_data.execution_metadata or {},
                    "timestamp": rich_data.timestamp.isoformat()
                }
            else:
                # Fallback to basic text output if rich extraction not available
                yield {"output": out}
    
    def _create_lightweight_json(self, rich_data) -> str:
        """FIXED: Create complete JSON with proper deduplication and observe_blocks."""
        import json
        
        # Create complete JSON object with all required fields
        lightweight_obj = {
            "step": rich_data.step_number,
            "type": rich_data.message_type,
            "timestamp": rich_data.timestamp.isoformat(),
        }
        
        # FIXED: Deduplicate execute_blocks and ensure observe_blocks are included
        if rich_data.has_code_execution and rich_data.code_blocks:
            # Deduplicate code blocks by code content
            seen_code = set()
            unique_execute_blocks = []
            
            for block in rich_data.code_blocks:
                code_signature = block.get('code', '')
                if code_signature not in seen_code:
                    seen_code.add(code_signature)
                    unique_execute_blocks.append(block)
                    print(f"✅ Added unique execute block: {len(code_signature)} chars")
                else:
                    print(f"🚫 Skipped duplicate execute block")
            
            lightweight_obj["execute_blocks"] = unique_execute_blocks
        
        # FIXED: Force extraction of observe_blocks from raw content
        if rich_data.parsed_content:
            if 'observations' in rich_data.parsed_content:
                lightweight_obj["observe_blocks"] = rich_data.parsed_content['observations']
                print(f"✅ Added {len(rich_data.parsed_content['observations'])} observe_blocks")
            else:
                print("⚠️ No observations in parsed_content, forcing extraction...")
                
                # FORCE extract observe blocks directly from raw content
                import re
                observe_pattern = r'<observe>(.*?)</observe>'
                observe_matches = re.findall(observe_pattern, rich_data.raw_content, re.DOTALL)
                
                if observe_matches:
                    forced_observations = []
                    for i, obs in enumerate(observe_matches):
                        forced_observations.append({
                            'type': 'observation',
                            'content': obs.strip(),
                            'has_errors': 'Error:' in obs,
                            'has_success': any(indicator in obs.lower() for indicator in ['successfully', 'completed', 'saved']),
                            'forced_extraction': True,
                            'observation_index': i
                        })
                    
                    lightweight_obj["observe_blocks"] = forced_observations
                    print(f"🔧 FORCED extraction of {len(forced_observations)} observe_blocks")
                    for i, obs in enumerate(observe_matches):
                        print(f"   Forced observe {i+1}: {obs.strip()[:100]}...")
                else:
                    print("❌ NO OBSERVE BLOCKS found even with forced extraction!")
            
            if 'solutions' in rich_data.parsed_content:
                lightweight_obj["solution_blocks"] = rich_data.parsed_content['solutions']
                print(f"✅ Added {len(rich_data.parsed_content['solutions'])} solution_blocks")
        
        # Include todo items
        if rich_data.has_todo_updates and rich_data.todo_items:
            lightweight_obj["todo_items"] = rich_data.todo_items
            print(f"✅ Added {len(rich_data.todo_items)} todo_items")
        
        # FIXED: Add file operations if detected
        if rich_data.file_operations:
            lightweight_obj["file_operations"] = rich_data.file_operations
            print(f"✅ Added {len(rich_data.file_operations)} file_operations")
        
        # FIXED: Add scientific data if detected
        if rich_data.scientific_data:
            lightweight_obj["scientific_data"] = rich_data.scientific_data
            print(f"✅ Added scientific_data")
        
        print(f"📤 JSON object keys: {list(lightweight_obj.keys())}")
        
        # Fast JSON conversion - no pretty printing to avoid blocking
        return json.dumps(lightweight_obj, separators=(',', ':'), default=str)
