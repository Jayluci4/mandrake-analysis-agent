    ...
    try {
      ...
      this.eventSource.onmessage = (event) => {
        try {
          ...
          this.handleEvent(data.type || data.event_type || 'message', data);
        } catch (error) {
          console.error('Failed to parse SSE message:', error, event.data);
        }
      };

      // Handle specific event types - both old and new formats
      const eventTypes = [
        'connected', 'planning', 'tool_call', 'tool_output', 'code_execution', 'visualization', 'final_result', 'error',
        'reasoning', 'tool_execution', 'observation', 'final_answer', 'todo_update', 'done'
      ];
...
const biomniApi = {
  ...
  chat: {
    sendMessage: async (message: string, sessionId: string = 'default', fileIds?: string[]) => {
      ...
      const url = `/api/chat/intelligent?${params.toString()}`;
      return { url };
    },
  },
