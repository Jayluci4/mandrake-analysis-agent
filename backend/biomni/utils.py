...
def write_python_code(request: str):
    ...
    # Use get_llm to properly handle Azure vs Anthropic
    from biomni.llm import get_llm
    
    # Use Azure if available, otherwise fall back to Claude
    if os.getenv("AZURE_OPENAI_API_KEY"):
        model = get_llm(model="azure-gpt-4.1", temperature=0.1)
    else:
