...
def _query_llm_for_api(prompt, schema, system_template, api_key=None, model="claude-3-5-haiku-20241022"):
    ...
    try:
        ...
        # If no API key provided and Azure key exists, use Azure model
        if not api_key and os.getenv("AZURE_OPENAI_API_KEY"):
            # Use Azure GPT-4.1 instead of Claude if Azure is configured
            llm = get_llm(model="azure-gpt-4.1", temperature=0.0, api_key=os.getenv("AZURE_OPENAI_API_KEY"))
        else:
