...
def get_llm(
    ...
    if source == "OpenAI":
        ...
        return AzureChatOpenAI(
            ...
            azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT") or os.getenv("OPENAI_ENDPOINT") or "https://mandrakebioworkswestus.openai.azure.com/",
            azure_deployment=model,
            openai_api_version=API_VERSION,
            temperature=temperature,
            max_tokens=32768,  # GPT-4.1 supports up to 128K context, setting reasonable output limit
            stop=stop_sequences,
        )
        ...
        # Try to use ChatVertexAI for proper Vertex AI integration
        try:
            from langchain_google_vertexai import ChatVertexAI
            
            # Configure for Gemini 2.5 Pro as per models.md
            return ChatVertexAI(
                model=model,
                temperature=temperature,
                project="bioagent-1753275345877",
                location="global",
                max_output_tokens=65535,  # As configured in models.md
                stop_sequences=stop_sequences,
            )
        except ImportError:
            # Fall back to OpenAI-compatible API if ChatVertexAI is not available
            try:
                from langchain_openai import ChatOpenAI
            except ImportError:
                raise ImportError(  # noqa: B904
                    "langchain-google-vertexai or langchain-openai package is required for Gemini models. "
                    "Install with: pip install langchain-google-vertexai"
                )
            
            # Use OpenAI-compatible endpoint with max_tokens
            return ChatOpenAI(
                model=model,
                temperature=temperature,
                api_key=os.getenv("GEMINI_API_KEY"),
                base_url="https://generativelanguage.googleapis.com/v1beta/openai/",
                max_tokens=65535,  # Add max_tokens for proper output handling
                stop_sequences=stop_sequences,
            )
